...
$scriptdir/../../../../bin/bvd-get.pl -T prostate_cancer,colon_cancer > $scriptdir/tmp.txt

result=$(diff $scriptdir/tmp.txt $scriptdir/expected_result)

if [ $? -eq 0 ]; then
    echo "All case_bvd_get_02_13 are correct !!! Congratz"
else
    echo "Something went wrong in case_bvd_get_02_13 testing. See below "
    echo "$result"
fi
