...
sub begin_add_tran
{
    ...
    $$self{_check_new_REF}->{new_REF_count}            = 0;
    $$self{_check_new_REF}->{new_REF_msg}              = [];
...
sub _count_new_REF
{
    ...
    if ($$self{current_variant}->{db_position_key} ne $$self{_check_new_REF}->{_last_db_position_key}) {
        $$self{_check_new_REF}->{new_REF_count}              += $$self{_check_new_REF}->{_tmp_new_REF_count}; 
        push(@{$$self{_check_new_REF}->{new_REF_msg}}, @{$$self{_check_new_REF}->{_tmp_new_REF_msg}}); 
        $$self{_check_new_REF}->{_db_reference_key_found}     = undef;
        $$self{_check_new_REF}->{_tmp_new_REF_count}          = 0;
        $$self{_check_new_REF}->{_tmp_new_REF_msg}            = [];
    }
...
sub commit_add
{
    ...
    if ($$self{_check_new_REF}->{new_REF_count}) {
        my @mymsg = ();
        push (@mymsg, "$$self{_check_new_REF}->{new_REF_count} new REF have been found and inserted into the database.\n");
        push (@mymsg, "CHROM\tPOS\tREF\tALT\n");
        push (@mymsg, "@{$$self{_check_new_REF}->{new_REF_msg}}");
        $self->warn(@mymsg);
        #$self->warn("Alleles with invalid reference might already be inserted!!!. $$self{_check_new_REF}->{new_REF_count} new REF have been added to the database.");
    }
