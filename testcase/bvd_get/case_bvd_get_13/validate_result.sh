...
cp $scriptdir/case_bvd_get_13_bvdb $scriptdir/../../../bin/DB/bvdb
cp $scriptdir/case_bvd_get_13_bvdb_chksum $scriptdir/../../../bin/DB/bvdb_chksum
...
if [ $? -eq 0 ]; then
    echo "All case_bvd_get_13 are correct !!! Congratz"
else
    echo "Something went wrong in case_bvd_get_13 testing. See below "
    echo "$result"
fi
